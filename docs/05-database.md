   ...
   └─ 选 9
      ...
      → 主菜单：
         选 4) Full installation (root)
            确认提示："All previous changes will be lost. Type 'DeleteAll' if you are sure"
            —— 逐字敲提示词，不是 y/N！
            随后自动串行完成：
              创建四个库 → 创建 mangos 用户授权 → 全量 Full_DB → 增量 Updates
              → core 侧 sql/updates → playerbots SQL（world、world/classic、characters 三组）
            中段再出现一次同类确认："Type 'Mangosbots' ..."——逐字敲 Mangosbots（大小写敏感）
```

⚠️ **确认词的两个真坑（都在实战中出现过）**：

1. 所有 `Type 'XXX' if you are sure:` 提示必须**逐字敲提示词**——直接回车被视为"否"，而且**静默跳过该环节、周围其他环节照常继续**；
2. `'Mangosbots'` 那次确认专门控制 playerbots 数据是否入库，漏敲后四库看似完美（数量多、内容多），实际 `ai_playerbot_*` 表数为 0——症状极隐蔽。补救：重跑脚本 `5) Advanced DB management → 8) Create and fill playerbots db`（同样逐字敲确认词）。
...
# playerbots 表（world 3+ 张、characters 6+ 张；少了=上面第 2 坑）
